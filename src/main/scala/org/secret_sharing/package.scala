...
package object secret_sharing {
  ...
  implicit def eitherMonad[Err] = new Monad[Either[Err, ?]]{
    override def flatMap[A, B](fa: Either[Err, A])(f: (A) => Either[Err, B]): Either[Err, B] = fa.right.flatMap(f)
    override def tailRecM[A, B](a: A)(f: (A) => Either[Err, Either[A, B]]): Either[Err, B] = f(a) match {
